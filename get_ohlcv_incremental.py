"""
kospi_code.mst 기반 증분 OHLCV 데이터 수집
기존 데이터와 병합하여 이어서 가져옵니다 (1997년부터 현재까지)
"""
import os
import time
import pandas as pd
...
def get_ohlcv_data_incremental(stock_code, output_dir, period="D", delay=0.1):
    ...
    try:
        ...
        if first_date is None:
            ...
            current_start = start_date_1997
            print(f"  기존 데이터 있음 (범위: {first_date.strftime('%Y-%m-%d')} ~ {last_date.strftime('%Y-%m-%d')})")
        ...
        print(f"    목표 날짜 범위: {current_start.strftime('%Y-%m-%d')} ~ {target_end_date.strftime('%Y-%m-%d')}")
        ...
        while current_start < target_end_date and iteration < max_iterations:
            ...
            print(f"    [{iteration}] 배치 조회: {current_start.strftime('%Y-%m-%d')} ~ {batch_end.strftime('%Y-%m-%d')}")
        ...
        if existing_df is not None and not existing_df.empty:
            ...
            combined_df = combined_df.sort_values('날짜', ascending=True).reset_index(drop=True)  # 오래된 것부터
        else:
            combined_df = new_df
...
def get_all_kospi_ohlcv_incremental(output_dir="ohlcv_data", period="D", delay=0.1):
    """
    kospi_code.mst 파일의 모든 종목에 대해 증분 OHLCV 데이터 수집
    ...
    print(f"\n총 {len(stock_codes)}개 종목의 OHLCV 데이터를 증분 수집합니다.")
    ...
    for idx, stock_code in enumerate(stock_codes, 1):
        ...
        success, new_count = get_ohlcv_data_incremental(
            stock_code, output_dir, period=period, delay=0  # 내부에서 delay 처리
        )
        ...
        if idx % 100 == 0:
            ...
            print(f"성공: {success_count}, 스킵: {skip_count}, 실패: {fail_count}, 새 데이터: {new_data_count}개\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='kospi_code.mst 기반 증분 OHLCV 데이터 수집 (1997년부터)')
    ...
    get_all_kospi_ohlcv_incremental(
